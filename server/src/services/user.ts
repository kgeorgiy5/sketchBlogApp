...
export const createNewPost = async (title: string, content: string, userId: SessionField<string>) => {
  const updateDate = new Date();
  const newPost = new Post({ title: title, content: content, updateDate: updateDate, userId: userId, numberOfLikes: 0 });
...
export const updatePost = async (postId: string, title: string, content: string, userId: SessionField<string>) => {
  const updateDate = new Date();
...
export const addLike = async (postId: mongoose.Types.ObjectId, userId: SessionField<string>) => {
  const post = await Post.findById(postId);
